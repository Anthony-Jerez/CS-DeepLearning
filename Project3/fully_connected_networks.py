...
"""
import torch
from p3_utils import softmax_loss, Solver
...
def adam(w, dw, config=None):
    ...
    hat_m = config['m'] / (1 - config['beta1'] ** config['t'])
    hat_v = config['v'] / (1 - config['beta2'] ** config['t'])
    next_w = w - config['learning_rate'] * hat_m / (torch.sqrt(hat_v) + config['epsilon'])
    #########################################################################
    #                              END OF YOUR CODE                         #
    #########################################################################
