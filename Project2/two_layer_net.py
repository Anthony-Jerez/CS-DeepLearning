...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    """
    The first stage of your neural network implementation: Run the forward pass
    ...
    hidden_before_relu = torch.mm(X, W1) + b1
    hidden = torch.clamp(hidden_before_relu, min=0)
...
def nn_forward_backward(
    ...
    ############################################################################
    # Replace "pass" statement with your code
    highest_scores = torch.max(scores, dim=1, keepdim=True).values
    exponentiated_scores = torch.exp(scores - highest_scores)
    softmax_probabilities = exponentiated_scores / torch.sum(exponentiated_scores, dim=1, keepdim=True)
    idx = torch.arange(N)
    correct_log_probabilities = torch.log(softmax_probabilities[idx, y]) * -1
    data_loss = torch.sum(correct_log_probabilities) / N
    regularization_loss = (torch.sum(W1 * W1) + torch.sum(W2 * W2)) * reg
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    grad_scores = softmax_probabilities.clone()
    grad_scores[idx, y] -= 1
    grad_scores /= N
    ...
    cond = (h1 <= 0)
    grad_h1[cond] = 0
    grads['W1'] = torch.mm(X.t(), grad_h1) + 2 * reg * W1
    grads['b1'] = torch.sum(grad_h1, dim=0)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_train(
    ...
    for it in range(num_iters):
        ...
        #########################################################################
        # Replace "pass" statement with your code
        for curr_weight in params:
            params[curr_weight] -= learning_rate * grads[curr_weight]
        #########################################################################
        #                             END OF YOUR CODE                          #
        #########################################################################
...
def nn_get_search_params():
    ...
    # hyperparameters that you evaluated in Part 2.1 of the Jupyter notebook. #
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [1e-4, 1e-2, 1e0, 1e2]
    hidden_sizes = [2, 8, 32, 128]
    regularization_strengths = [0, 1e-5, 1e-1, 1e-3]
...
def find_best_net(
    ...
    # the ones in the Jupyter notebook; these visualizations will have          #
    # significant qualitative differences from the ones we saw above for        #
    # poorly tuned network.                                                     #
    #                                                                           #
        ...
        for lr in learning_rates:
            for hs in hidden_sizes:
                for lrd in learning_rate_decays:
                    net = TwoLayerNet(data_dict['X_train'].shape[1], hs, 10, dtype=data_dict['X_train'].dtype, device='cpu')
                    stats = net.train(
                        data_dict['X_train'], 
                        data_dict['y_train'], 
                        data_dict['X_val'], 
