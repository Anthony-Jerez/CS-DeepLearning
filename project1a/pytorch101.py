...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    x_temp = x.view(2, 3, 4)
    x_transpose = x_temp.transpose(0, 1)
    y = x_transpose.reshape(3, 8)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y = x.clone()
    minimum_indices = torch.argmin(x, dim=1)
    y[torch.arange(x.shape[0]), minimum_indices] = 0
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    M, N = x.shape
    mu = x.sum(dim=0) / M
    s = torch.sqrt(((x - mu) ** 2).sum(dim=0) / (M - 1))
    y = (x - mu) / s
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def mm_on_gpu(x: Tensor, w: Tensor) -> Tensor:
    ...
    When you move the tensor to GPU, use the "your_tensor.cuda()" operation for 
    CUDA, or first get the device, e.g., your_device = torch.device("mps"), and 
    then move the tensor: your_tensor = your_tensor.to(your_device).
...
def challenge_get_uniques(x: torch.Tensor) -> Tuple[Tensor, Tensor]:
    ...
    uniques = torch.zeros(x.shape[0], dtype=x.dtype)
    indices = torch.zeros(x.shape[0], dtype=torch.long)
    count = 0
