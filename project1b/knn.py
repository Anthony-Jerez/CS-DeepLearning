...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        for j in range(num_test):
            difference = x_train_flattened[i] - x_test_flattened[j]
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_test_flattened = x_test.view(num_test, -1)
    for j in range(num_test):
        difference = x_train_flattened - x_test_flattened[j]
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train_flattened = x_train.view(num_train, -1)
    x_test_flattened = x_test.view(num_test, -1)
    x_train_square_sum = torch.sum(x_train_flattened ** 2, dim=1, keepdim=True)
    x_test_square_sum = torch.sum(x_test_flattened ** 2, dim=1)
    dists = x_train_square_sum + x_test_square_sum - 2 * torch.mm(x_train_flattened, x_test_flattened.t())
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    # Replace "pass" statement with your code
    for j in range(num_test):
        distances_to_jtest = dists[:, j]
        _, idx = torch.topk(distances_to_jtest, k, largest=False)
        knn_labels = y_train[idx]
        unique_class_labels, counts = torch.unique(knn_labels, return_counts=True)
        max_count = torch.max(counts)
        potential_labels = unique_class_labels[counts == max_count]
        predicted_label = torch.min(potential_labels)
...
def knn_cross_validate(
    ...
    for i in range(num_folds):
        x_validation_fold, y_validation_fold = x_train_folds[i], y_train_folds[i]
        x_train_fold_temp, y_train_fold_temp = [], []
        for j in range(num_folds):
            if i != j:
                x_train_fold_temp.append(x_train_folds[j])
                y_train_fold_temp.append(y_train_folds[j])
        x_train_remaining = torch.cat(x_train_fold_temp)
        y_train_remaining = torch.cat(y_train_fold_temp)
        knn_classifier = KnnClassifier(x_train_remaining, y_train_remaining)
        for k in k_choices:
            y_validation_pred = knn_classifier.predict(x_validation_fold, k)
            accuracy = torch.mean((y_validation_fold == y_validation_pred) * 1.0).item()
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k, accs in k_to_accuracies.items():
        ...
        if mean_accuracy > best_mean_accuracy:
            ...
            best_k = k
        elif mean_accuracy == best_mean_accuracy and k < best_k:
            best_k = k
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
