"""
Utility functions used in Project 3
"""
import torch
import torchvision
...
def corr2d(x,k):
    ...
    OH = H-HH+1
    OW = W-WW+1
    y = torch.zeros((OH, OW),device=x.device,dtype=x.dtype)
    for i in range(OH):
        for j in range(OW):
            y[i,j]=(x[i:i+HH,j:j+WW]*k).sum()
    return y

def dilate2d(x,S:int):
    """
    Dilates a 2d tensor.
    Inputs:
    - x: Array of data, of shape (N, M)
    - S: Stride size, an integer
    Returns:
    - d: A dilated version of x with shape (N +(N-1)(S-1),M+(M-1)(S-1))
    """
    if (len(x.shape))!=2:
        raise ValueError("A 2D tensor is required, received a tensor of order",len(x.shape))
    if type(S)!=int or S < 1:
        raise ValueError("Stride must be an integer >=1, received",S)
    #if stride is one, no dilation is necessary
    if S ==1:
        return x
    N,M = x.shape
    d = torch.zeros((N +(N-1)*(S-1),M+(M-1)*(S-1)),device=x.device,dtype=x.dtype)
    Si=0
    for i in range(N):
        Sj=0
        for j in range(M):
...
def stacked_dilate2d(x,S:int):
    """
    Dilates a 3d stack of 2d tensors.
    Inputs:
    - x: Array of data, of shape (F, N, M)
    - S: Stride size, an integer
    Returns:
    - d: A dilated version of x of shape (F,N +(N-1)(S-1),M+(M-1)(S-1))
    """
    if (len(x.shape))!=3:
        raise ValueError("A 3D tensor is required, received a tensor of order",len(x.shape))
    if type(S)!=int or S < 1:
        raise ValueError("Stride must be an integer >=1, received",S)
    #if stride is one, no dilation is necessary
    if S ==1:
        return x
    F, N,M = x.shape
    d = torch.zeros((F,N +(N-1)*(S-1),M+(M-1)*(S-1)),device=x.device,dtype=x.dtype)
